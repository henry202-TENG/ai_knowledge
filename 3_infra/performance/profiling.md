...
**LLM Profiling & Benchmarking** 是**系統性測量和分析** LLM 性能的技術：
...
│  │  資源利用率:                                                 │   │
│  │  ┌─────────────────────────────────────────────────────┐  │   │
│  │  │  GPU Utilization: 計算使用率                         │  │   │
│  │  │  Memory Bandwidth: 記憶體頻寬                        │   │
│  │  │  NVLink Utilization: GPU 互聯頻寬                    │  │   │
│  │  └─────────────────────────────────────────────────────┘  │   │
│  │                                                              │   │
│  │  成本效率:                                                   │   │
│  │  ┌─────────────────────────────────────────────────────┐  │   │
...
│  │  2. 框架層: 批量處理、緩存命中率                            │   │
│  │  3. 模型層: 前向/反向傳播時間                              │   │
│  │  4. 內核層: Attention、Linear 計算時間                     │   │
│  │  5. 硬件層: GPU 利用率、記憶體帶寬                        │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  基準測試類型:                                                        │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  Micro-benchmark: 單一操作延遲                            │   │
│  │  Macro-benchmark: 完整 pipeline 性能                       │   │
│  │  Stress test: 極限負載測試                                │   │
│  │  Soak test: 長時間穩定性測試                             │   │
│  │  Benchmark suite: MLPerf、LM Harness                     │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  核心挑戰:                                                           │
│  1.  Variability: GPU 性能波動                                    │
│  2.  Warm-up: 首次運行需要預熱                                    │
│  3.  Caching: KV Cache 影響測量準確性                            │
│  4.  Noise: 共享環境干擾                                           │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
