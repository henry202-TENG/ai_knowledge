...
**Kubernetes for ML** 是使用 **K8s 容器編排平台**部署和管理 ML 工作負載的技術：
...
│  核心組件:                                                           │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  GPU Operator: 自動管理 GPU 資源                            │   │
│  │  Device Plugin: NVIDIA GPU 發現和調度                       │   │
│  │  Scheduler Extender: 自定義調度策略                         │   │
│  │  Persistent Volumes: 模型和數據持久化                       │   │
│  │  Network Policies: 隔離和安全性                             │   │
│  │  Horizontal Pod Autoscaler: CPU/內存/自定義指標擴展        │   │
│  │  KEDA: 事件驅動的 Serverless 擴展                          │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  核心挑戰:                                                           │
│  1. GPU 調度: 確保 GPU 正確分配和隔離                               │   │
│  2. 資源配額: 合理分配 GPU/內存/CPU                                │   │
│  3. 分散式訓練: 多 Pod 協調和通訊                                   │   │
│  4. 模型部署: 版本管理、A/B 測試                                   │   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
