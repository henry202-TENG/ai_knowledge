...
**Model Distillation (模型蒸餾)** 是一種**知識遷移**技術，讓小模型學習大模型的行為：

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Model Distillation 核心思想                       │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  問題:                                                               │
│  - 大模型 (70B) 能力強但推理成本高                                   │
...
│  │  老師 (Teacher):                                            │   │
│  │  - 輸入: "What is AI?"                                       │   │
│  │  - 輸出: P(AI) = 0.85, P(ML) = 0.10, P(Deep Learning)=0.05 │   │
│  │                                                              │   │
│  │  學生 (Student):                                             │   │
│  │  - 輸入: "What is AI?"                                       │   │
│  │  - 蒸餾學習: 盡可能匹配老師的機率分佈                        │   │
│  │  - 輸出: P(AI) = 0.83, P(ML) = 0.11, P(Deep Learning)=0.06 │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  關鍵: 「暗知識」(Dark Knowledge)                                    │
│  - 標準訓練只學「正確答案」                                          │
│  - 蒸餾學習「錯誤答案的相對順序」                                    │
│  - 例如: AI 錯了，ML 是第二可能的答案                                │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

**為何有效**:
1. **暗知識**: 學生學習老師對錯誤答案的「信心度排序」
2. **溫度縮放**: 高溫下 soft labels 攜帶更多資訊
3. **多層次學習**: 可蒸餾特徵、注意力、輸出
