...
**Mamba/SSM** 是一種基於**狀態空間模型 (State Space Model)** 的新型序列處理架構：
...
**為何重要**:
1. **突破記憶體瓶頸**: Transformer 的 KV Cache 在長序列時爆炸
2. **硬體友好**: 避免了 Attention 的記憶體訪問模式
3. **選擇性機制**: Mamba 能動態決定保留/忽略哪些資訊
